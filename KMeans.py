...
FILENAME = 'resources/s1.txt'


def euclidean_distance(x, y, centroid):
    return math.sqrt((x - centroid[0]) ** 2 + (y - centroid[1]) ** 2)


def action(points, x_cord, y_cord, centroids, clusters):
    for i in range(len(points)):
        min_distance = euclidean_distance(x_cord[i], y_cord[i], centroids[0])
        min_index = 0
        for j in range(len(centroids)):
            if euclidean_distance(x_cord[i], y_cord[i], centroids[j]) < min_distance:
                min_distance = euclidean_distance(x_cord[i], y_cord[i], centroids[j])
                min_index = j
